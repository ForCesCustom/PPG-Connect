...
native People Playground recovery dialog with the likely causes and an
**OPEN CONNECT ON GITHUB** button. The direct URL is also included in the
standard in-game notification. The standard mod scanner rejects file-path
inspection, so the main BepInEx runtime performs the exact file-name check when
it is available.
