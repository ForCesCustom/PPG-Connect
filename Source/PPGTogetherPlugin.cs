...
namespace PPGTogether.BepInEx
{
    ...
    public sealed partial class PPGTogetherPlugin : BaseUnityPlugin
    {
        ...
        private const int MaximumCompoundBodiesPerSnapshot = 48;
        // At 20 Hz this bounds a 500-object world to 480 root updates per
        // second instead of attempting 10,000+ relay packets per second.
        private const int MaximumSnapshotRootsPerTick = 24;
        private const int WorldBaselineChunkSize = 16;
        private const float WorldBaselineIntervalSeconds = 0.45f;
        private const int WorldBaselineCycles = 2;
        ...
        private int snapshotRootCursor;
        ...
        private readonly Dictionary<ulong, uint> clientRootSnapshotTicks = new Dictionary<ulong, uint>();
        private readonly Dictionary<ulong, uint> clientRigSnapshotTicks = new Dictionary<ulong, uint>();
        private bool suppressClientGrabUntilMouseUp;
        ...
        {
            Instance = this;
            PPGTogetherIdentity.DestroyedCallback = OnNetworkIdentityDestroyed;
            EnsureRuntimeMarker();
            ...
            supportGithubReleaseUrlSetting = Config.Bind("Support", "GitHub Download URL", ConnectSupportLinks.PublishedReleaseUrl, "Canonical HTTPS direct download used by the missing-files recovery notice. Connect automatically migrates old saved release links to the matching current package.");
            if (!string.Equals(supportGithubReleaseUrlSetting.Value, ConnectSupportLinks.PublishedReleaseUrl, StringComparison.Ordinal))
            {
                supportGithubReleaseUrlSetting.Value = ConnectSupportLinks.PublishedReleaseUrl;
                Logger.LogInfo("[Connect][Install] Updated the saved recovery link to the matching Connect release.");
            }
        ...
        private void Update()
        {
            ...
            if (!relayAccessInitialised)
            {
                ...
                if (!lobby.HasValue) SetStatus("Steam is ready. Create a lobby or join a friend's invitation.");
            }
            transport.Pump();
            avatars.Pump();
        ...
        private void OnGUI()
        {
            if (menuReveal > 0.001f) DrawConnectMenu();
        ...
        private string GitHubReleaseUrl
        {
            get
            {
                return ConnectSupportLinks.PublishedReleaseUrl;
            }
        }
        ...
        {
            if (!patchApplied) { SetStatus("Required game patches failed. Multiplayer is disabled; check the Connect log and game version."); return; }
        ...
        private void SendHello()
        {
            ...
            writer.String(LocalGameVersion);
        ...
        private void ProcessReceivedPackets()
        {
            ...
            while (processed++ < 64 && transport.TryDequeue(out packet))
            {
                ...
                if (envelope.Channel != WireChannel.Cursor && envelope.Channel != WireChannel.Snapshot && envelope.Type != WireMessage.GrabUpdate)
        ...
        {
            // Only the authenticated connection may issue state changes. In
            // particular, a guest must never end/start the host's session.
            if (!lobby.HasValue) return;
            if (IsHost)
            {
                if (envelope.Type != WireMessage.Hello)
                {
                    Peer sender;
                    if (!peers.TryGetValue(packet.SteamId, out sender) || sender.PeerId != envelope.PeerId || sender.Connection.Id != packet.Connection.Id) return;
                    if (envelope.Type != WireMessage.Cursor && envelope.Type != WireMessage.ClientMapStatus &&
                        envelope.Type != WireMessage.SpawnRequest && envelope.Type != WireMessage.GrabBegin &&
                        envelope.Type != WireMessage.GrabUpdate && envelope.Type != WireMessage.GrabEnd &&
                        envelope.Type != WireMessage.InteractionRequest && envelope.Type != WireMessage.WorldCommand) return;
                    if (envelope.Channel == WireChannel.World && (!sessionActive || sender.MapStatus != PeerMapStatus.Playing)) return;
                }
            }
            else if (packet.SteamId != (ulong)lobby.Value.Owner.Id) return;
            if (envelope.Channel == WireChannel.World || envelope.Channel == WireChannel.Snapshot)
            {
                Reader scoped = new Reader(envelope.Payload); uint epoch; byte[] contents;
                if (!scoped.UInt(out epoch) || epoch != (IsHost ? worldEpoch : clientWorldEpoch) || !scoped.Raw(scoped.Remaining, out contents)) return;
            ...
            if (envelope.Type == WireMessage.SessionStarted && !IsHost) { HandleSessionStarted(); return; }
            if (envelope.Type == WireMessage.SessionEnding && !IsHost) { sessionActive = false; ResetWorldLifecycle(); ClearBotCursors(); SetStatus("Host ended the session."); return; }
            ...
            if (envelope.Type == WireMessage.ObjectState && !IsHost) { if (sessionActive) HandleObjectState(envelope); return; }
            if (envelope.Type == WireMessage.WorldManifest && !IsHost) { if (sessionActive) HandleWorldManifest(envelope); return; }
            if (envelope.Type == WireMessage.GlobalState && !IsHost) { if (sessionActive) HandleGlobalState(envelope); return; }
        ...
        private void HandleHello(ReceivedPacket packet, Envelope envelope)
        {
            ...
            if (gameVersion != LocalGameVersion) { Logger.LogWarning("[Connect][Transport] Rejected client with People Playground " + SafeName(gameVersion) + "; host is " + LocalGameVersion + "."); SendReject(packet.Connection, "Wrong People Playground version"); return; }
        ...
        private void HandleMapLoad(Envelope envelope)
        {
            ...
            uint epoch;
            string identity;
            if (!reader.UInt(out epoch) || epoch == 0 || !reader.String(out identity) || reader.Remaining != 0 || !IsValidMapIdentity(identity))
            {
                SetStatus("Host sent an invalid map identity.");
                return;
            }

            if (PrepareClientWorldEpoch(epoch)) QueueClientMapLoad(identity, "Steam Relay");
        }
        ...
        private void HandleClientMapStatus(ReceivedPacket packet, Envelope envelope)
        {
            ...
            uint epoch;
            string mapIdentity;
            if (envelope.PeerId != peer.PeerId || !reader.UInt(out epoch) || !reader.Byte(out rawStatus) || !reader.String(out mapIdentity) || reader.Remaining != 0 ||
            ...
            if (!sessionActive || epoch != worldEpoch || !string.Equals(activeMapIdentity, mapIdentity, StringComparison.Ordinal))
            {
        ...
        internal void OnLocalMapLoaded(MapLoaderBehaviour loader)
        {
            ...
            {
                if (clientForceMapReload && !clientMapLoadIssued) return;
                clientEpochLoadCallbackObserved = true;
        ...
        private void UpdateMapSynchronisation()
        {
            ...
            {
                string hostMap;
                if (TryGetCurrentMapIdentity(out hostMap) && !string.Equals(hostMap, activeMapIdentity, StringComparison.Ordinal) && IsMapActuallyLoaded(hostMap))
            ...
            if (Time.unscaledTime >= nextClientMapProbeAt)
            {
                ...
                if ((!clientForceMapReload || clientEpochLoadCallbackObserved) && IsRequestedClientMapActuallyLoaded())
        ...
        private void ApplyLobbyMapDirective(Lobby source)
        {
            ...
            if ((state == "loading" || state == "playing") && IsValidMapIdentity(identity) &&
                uint.TryParse(source.GetData("ppgt_world_epoch"), out epoch) && epoch != 0)
            {
                ...
                if (!PrepareClientWorldEpoch(epoch)) return;
        ...
        private void QueueClientMapLoad(string identity, string source)
        {
            ...
            // Stop accepting world input and delayed snapshots immediately.
            // The host will resend the registered baseline only after this map
            // has a verified local root and reports PLAYING again.
            if (sessionActive)
            {
                EndClientGrab();
        ...
        {
            if (IsHost || sessionActive || clientWorldEpoch == 0 || !clientSessionStartReceived || clientMapLoadPending || clientMapSceneTransitionPending) return;
        ...
        private void TryBeginClientMapLoad()
        {
            ...
            if (!clientForceMapReload && !clientMapSceneTransitionPending && IsSandboxSceneActive() && IsRequestedClientMapActuallyLoaded())
            {
            ...
            // In People Playground 1.27.17 MapLoadOverride is honoured only
        ...
        private void BeginHostSession(string mapIdentity)
        {
            ...
            lobby.Value.SetData("ppgt_world_epoch", worldEpoch.ToString());
        ...
        // current host map. The compact bounded resend is deliberately repeated
        // twice: a guest can recover from a transient catalog/relay race without
        // letting a large world flood reliable traffic forever.
        private void BeginRegisteredWorldBaseline(Peer peer)
        {
            if (!IsHost || peer == null || peer.Connection == null) return;
            peer.BaselineCursor = 0;
            peer.BaselineCyclesRemaining = WorldBaselineCycles;
            peer.NextBaselineAt = 0f;
            Logger.LogInfo("[Connect][Sync] Queued " + WorldBaselineCycles + " registered-world reconciliation pass(es) for " + peer.Name + ".");
        }

        private void PumpRegisteredWorldBaselines()
        {
            if (!IsHost || !sessionActive) return;
            float now = Time.unscaledTime;
            foreach (Peer peer in peers.Values)
            {
                if (peer == null || peer.BaselineCyclesRemaining <= 0 || peer.Connection == null || peer.MapStatus != PeerMapStatus.Playing ||
                    !string.Equals(peer.MapIdentity, activeMapIdentity, StringComparison.Ordinal) || now < peer.NextBaselineAt)
                    continue;
                SendRegisteredWorldBaselineChunk(peer);
                peer.NextBaselineAt = now + WorldBaselineIntervalSeconds;
            }
        }

        private void SendRegisteredWorldBaselineChunk(Peer peer)
        {
            List<PPGTogetherIdentity> identities = new List<PPGTogetherIdentity>();
            foreach (PPGTogetherIdentity identity in registry.All())
                if (identity != null && identity.gameObject != null && !string.IsNullOrEmpty(identity.SpawnKey))
                    identities.Add(identity);
            if (identities.Count == 0)
            {
                peer.BaselineCyclesRemaining = 0;
                return;
            }
            identities.Sort(delegate(PPGTogetherIdentity left, PPGTogetherIdentity right)
            {
                return left.NetId < right.NetId ? -1 : (left.NetId > right.NetId ? 1 : 0);
            });
            int count = Mathf.Min(WorldBaselineChunkSize, identities.Count);
            int start = peer.BaselineCursor % identities.Count;
            for (int i = 0; i < count; i++)
            {
                PPGTogetherIdentity identity = identities[(start + i) % identities.Count];
                SendSpawnToConnection(peer.Connection, peer.PeerId, identity, identity.gameObject);
            }
            int next = start + count;
            peer.BaselineCursor = next % identities.Count;
            if (next >= identities.Count)
            {
                peer.BaselineCyclesRemaining--;
                if (peer.BaselineCyclesRemaining == 0)
                    Logger.LogInfo("[Connect][Sync] Registered-world reconciliation complete for " + peer.Name + ".");
            }
        }
        ...
        private void ResetNetworkWorldForMapTransition()
        {
            ...
            hostGrabId = 0;
            hostGrabToken = 0;
            snapshotRootCursor = 0;
            suppressClientGrabUntilMouseUp = false;
            clientRootSnapshotTicks.Clear();
            clientRigSnapshotTicks.Clear();
            hostCatalogSpawnKeys.Clear();
            registry.Clear();
        ...
        {
            if (IsHost || !lobby.HasValue || clientWorldEpoch == 0 || clientPeerId == 0 || transport == null || !transport.Connected || !IsValidMapIdentity(mapIdentity)) return;
            Writer writer = new Writer(80);
            writer.UInt(clientWorldEpoch);
        ...
        {
            if (menuVisible || Global.main == null || Global.main.UILock)
            {
                if (clientGrabId != 0) EndClientGrab();
                if (hostGrabId != 0) { grabs.ReleasePeer(0); hostGrabId = 0; hostGrabToken = 0; }
                return;
            }
            ...
            if (suppressClientGrabUntilMouseUp)
            {
                if (!Input.GetMouseButton(0)) suppressClientGrabUntilMouseUp = false;
                else return;
            }
            bool dragTool = Global.main.ToolControllerBehaviour != null && Global.main.ToolControllerBehaviour.CurrentTool is DragTool;
            if (!dragTool) { if (clientGrabId != 0) EndClientGrab(); return; }
        ...
        private void SendGrabBegin(Vector2 point)
        {
            ...
            {
                SetStatus("Grab a Connect object, not a local-only object.");
                return;
            }
            Writer writer = new Writer(20); writer.ULong(identity.NetId); writer.Float(point.x); writer.Float(point.y);
        ...
        private void HandleGrabBegin(ReceivedPacket packet, Envelope envelope)
        {
            ...
            Reader reader = new Reader(envelope.Payload); ulong id; float x; float y;
            if (!reader.ULong(out id) || !reader.Float(out x) || !reader.Float(out y) || reader.Remaining != 0) return;
            Vector2 target = new Vector2(x, y);
            if (!IsPeerNearPoint(peer, target)) { Logger.LogInfo("[Connect][Grab] Denied " + peer.Name + ": cursor is too far from the requested point."); SendGrabDenied(packet.Connection, "Requested point is too far from your network cursor."); return; }
            ActiveGrab grab; string denied;
            if (!grabs.TryBegin(peer.PeerId, id, target, hostTick, out grab, out denied)) { Logger.LogInfo("[Connect][Grab] Denied " + peer.Name + ": " + denied + "."); SendGrabDenied(packet.Connection, denied); return; }
        ...
        private void BroadcastSnapshots()
        {
            ...
            int count = Mathf.Min(MaximumSnapshotRootsPerTick, identities.Count);
            int start = snapshotRootCursor % identities.Count;
            for (int selected = 0; selected < count; selected++)
            {
                PPGTogetherIdentity identity = identities[(start + selected) % identities.Count];
                ...
                // Physics2D bodies. Send one bounded packet per spawned root,
                // not one Steam message per limb: the latter can starve a
                // reliable Spawn behind a transient snapshot flood.
                PhysicalBehaviour[] bodies = identity.GetComponentsInChildren<PhysicalBehaviour>(true);
                List<PhysicalBehaviour> parts = new List<PhysicalBehaviour>();
                ...
                for (int i = 0; i < bodies.Length; i++)
                {
                    ...
                    parts.Add(part);
                    paths.Add(path);
                    if (parts.Count >= MaximumCompoundBodiesPerSnapshot) break;
                }
                if (parts.Count > 0) BroadcastRigSnapshot(identity.NetId, paths, parts);
            }
            snapshotRootCursor = (start + count) % identities.Count;
        }
        ...
        private void BroadcastRigSnapshot(ulong netId, List<string> paths, List<PhysicalBehaviour> parts)
        {
            if (paths == null || parts == null || paths.Count == 0 || paths.Count != parts.Count || paths.Count > MaximumCompoundBodiesPerSnapshot) return;
            Writer writer = new Writer(32 + paths.Count * 48);
            writer.ULong(netId);
            writer.Byte((byte)paths.Count);
            for (int i = 0; i < paths.Count; i++)
            {
                PhysicalBehaviour part = parts[i];
                if (part == null || part.rigidbody == null) return;
                Rigidbody2D body = part.rigidbody;
                writer.String(paths[i]); writer.Float(body.position.x); writer.Float(body.position.y); writer.Float(body.rotation);
                writer.Float(body.velocity.x); writer.Float(body.velocity.y); writer.Float(body.angularVelocity); writer.Bool(body.simulated); writer.Bool(!body.IsAwake());
            }
        ...
        private void HandleSnapshot(Envelope envelope)
        {
            ...
            uint previousTick;
            if (clientRootSnapshotTicks.TryGetValue(id, out previousTick) && !CursorSequence.IsNewer(envelope.Tick, previousTick)) return;
            clientRootSnapshotTicks[id] = envelope.Tick;
        ...
        {
            Reader reader = new Reader(envelope.Payload); ulong id; byte count;
            if (!reader.ULong(out id) || !reader.Byte(out count) || count == 0 || count > MaximumCompoundBodiesPerSnapshot) return;
            RigSnapshotState[] states = new RigSnapshotState[count];
            for (int i = 0; i < count; i++)
            {
                RigSnapshotState state = new RigSnapshotState();
                if (!reader.String(out state.Path) || !reader.Float(out state.X) || !reader.Float(out state.Y) || !reader.Float(out state.Rotation) || !reader.Float(out state.VelocityX) || !reader.Float(out state.VelocityY) || !reader.Float(out state.AngularVelocity) || !reader.Bool(out state.Simulated) || !reader.Bool(out state.Sleeping)) return;
                states[i] = state;
            }
            if (reader.Remaining != 0) return;
            uint previousTick;
            if (clientRigSnapshotTicks.TryGetValue(id, out previousTick) && !CursorSequence.IsNewer(envelope.Tick, previousTick)) return;
            clientRigSnapshotTicks[id] = envelope.Tick;
            PPGTogetherIdentity identity;
            if (!registry.TryGet(id, out identity) || identity == null) return;
            for (int i = 0; i < states.Length; i++)
            {
                RigSnapshotState state = states[i];
                Transform target = FindTransformByIndexPath(identity.transform, state.Path);
                if (target == null) continue;
                PhysicalBehaviour physical = target.GetComponent<PhysicalBehaviour>();
                if (physical == null || physical.rigidbody == null) continue;
                ApplyRigidbodySnapshot(physical.rigidbody, state.X, state.Y, state.Rotation, state.VelocityX, state.VelocityY, state.AngularVelocity, state.Simulated, state.Sleeping);
            }
        }
        ...
        {
            if (body == null) return;
            // A replica is visual state owned by the host. It must not run a
            // second local ragdoll simulation between soft velocity hints;
            // applying exact poses keeps both screens in the same world.
            body.bodyType = RigidbodyType2D.Kinematic;
            body.position = new Vector2(x, y);
            body.rotation = rotation;
            // Velocity is informative on the wire, but a kinematic replica must
            // not drift between two exact host poses under local simulation.
        ...
        internal void OnItemSpawned(object sender, UserSpawnEventArgs args)
        {
            ...
            if (identity != null)
            {
                ReplicatedObjectState.Prime(identity);
        ...
        {
            if (identity == null || string.IsNullOrEmpty(identity.SpawnKey)) return;
        ...
        private void HandleSpawn(Envelope envelope)
        {
            ...
            if (!reader.ULong(out id) || id == 0 || !reader.String(out key) || !reader.Float(out x) || !reader.Float(out y) || !reader.Float(out rotation) || !reader.Float(out sx) || !reader.Float(out sy) || !reader.Float(out sz) || reader.Remaining != 0) return;
            ...
            PPGTogetherIdentity replica = registry.RegisterReplica(instance, id, key);
            if (replica == null) { Logger.LogWarning("[Connect][Spawn] Guest could not register Spawn key=" + SafeName(key) + ", netId=" + id + "."); Destroy(instance); return; }
            ReplicatedObjectState.Prime(replica);
            FreezeReplicaPhysics(instance);
        ...
        private void HandleDespawn(Envelope envelope)
        {
            ...
            {
                DestroyClientReplica(identity);
            }
        }

        internal void OnNetworkIdentityDestroyed(PPGTogetherIdentity identity)
        {
            if (identity == null || identity.NetId == 0) return;
            PPGTogetherIdentity registered;
            if (!registry.TryGet(identity.NetId, out registered) || registered != identity) return;
            ulong id = identity.NetId;
            ReplicatedObjectState.Forget(id);
            registry.Remove(identity);
            RemoveClientSnapshotTracking(id);
            if (!IsHost || !sessionActive) return;
            Writer writer = new Writer(8); writer.ULong(id);
            Broadcast(WireMessage.Despawn, WireChannel.World, writer.ToArray(), true);
            Logger.LogInfo("[Connect][Spawn] Broadcast Despawn for destroyed root netId=" + id + ".");
        }
        ...
        private void RemoveClientSnapshotTracking(ulong id)
        {
            ...
            clientRootSnapshotTicks.Remove(id);
            clientRigSnapshotTicks.Remove(id);
        }

        private static void FreezeReplicaPhysics(GameObject instance)
        {
            if (instance == null) return;
        ...
        private GameObject SpawnAndReplicate(SpawnableAsset asset, string spawnKey, Vector2 position, bool flipped)
        {
            ...
            if (!registry.TryGet(created, out existing))
            {
                ...
                if (existing != null) { ReplicatedObjectState.Prime(existing); BroadcastSpawn(existing, created); }
        ...
        {
            // On the current PPG build PerformInstantiation uses
            // CatalogBehaviour.SelectedItem.Prefab in its normal branch, not
            // the Prefab on the asset parameter. A guest request for Android
            // would therefore instantiate whichever item the host had last
            // selected (and vice versa on the guest). Scope the selected item
            // to this synchronous vanilla flow and restore it immediately.
            if (asset == null) return null;
            CatalogBehaviour catalog = CatalogBehaviour.Main;
            if (catalog != null)
            {
                SpawnableAsset previous = catalog.SelectedItem;
                bool requestedFlip = flipped;
                try
                {
                    catalog.SelectedItem = asset;
                ...
                {
                    Logger.LogWarning("[Connect][Spawn] Vanilla instantiation failed for " + SafeName(asset.NameToOrderBy) + ": " + exception.GetType().Name + ".");
                    return null;
                }
                finally { catalog.SelectedItem = previous; }
            }
        ...
        {
            // The click which places an item in the Tab catalog is not a grab
            // attempt. Suppress the matching mouse-down until release so it
            // cannot generate a bogus GrabBegin before the host creates it.
            suppressClientGrabUntilMouseUp = true;
        ...
        internal void HandleClientBlockedToolInput()
        {
            ...
            if (menuVisible || Global.main == null || Global.main.UILock) { EndClientActivations(); return; }
        ...
        private void HandleInteractionRequest(ReceivedPacket packet, Envelope envelope)
        {
            ...
            if (!activationAction && action != NetworkInteraction.Delete && !hostGuestsCanActivateSetting.Value)
            {
                SendActionDenied(packet.Connection, "Host disabled guest object changes."); return;
            }
            bool applied = action == NetworkInteraction.Activate ? HostActivate(identity) :
                action == NetworkInteraction.Delete ? HostDelete(identity) : ApplySharedContextAction(identity, action);
        ...
        private bool IsPeerNearObject(Peer peer, PPGTogetherIdentity identity)
        {
            ...
            foreach (PhysicalBehaviour physical in ReplicatedObjectState.GetPhysicalParts(identity))
                if (physical != null && physical.gameObject.activeInHierarchy && ((Vector2)physical.transform.position - cursor.Target).sqrMagnitude <= 36f) return true;
            return false;
        }

        private bool IsPeerNearPoint(Peer peer, Vector2 point)
        {
            if (peer == null) return false;
            RemoteCursor cursor;
            return cursors.TryGetValue(peer.PeerId, out cursor) && cursor != null && (point - cursor.Target).sqrMagnitude <= 36f;
        }

        private static PPGTogetherIdentity GetIdentity(PhysicalBehaviour physical)
        {
            return ReplicatedObjectState.FindIdentity(physical);
        }
        ...
        private void Cleanup(bool leaveSteamLobby)
        {
            ...
            sessionActive = false; clientPeerId = 0; clientGrabId = 0; clientGrabToken = 0; hostGrabId = 0; hostGrabToken = 0; snapshotRootCursor = 0; suppressClientGrabUntilMouseUp = false;
            ...
            grabs.Clear(); continuousActivations.Clear(); clientHeldActivationRoots.Clear(); clientRootSnapshotTicks.Clear(); clientRigSnapshotTicks.Clear(); hostCatalogSpawnKeys.Clear(); registry.Clear(); peers.Clear(); cursors.Clear(); avatars.Clear(); guestSpawnWindows.Clear(); guestInteractionWindows.Clear(); remoteHostSettings = new HostSettingsView();
        ...
        private void WriteLobbyMetadata()
        {
            ...
            value.SetData("ppgt_game_version", LocalGameVersion);
        ...
        private void DrawMenuHeader(Rect panel)
        {
            ...
            GUI.Label(new Rect(panel.x + panel.width - 202f, panel.y + 53f, 184f, 15f), "v" + PluginVersion + "  ·  GAME " + LocalGameVersion, ui.Small);
        }
        ...
        internal bool ShouldBlockVanillaWorldInput { get { return patchApplied && !IsHost && lobby.HasValue && (sessionActive || ClientMapTransitionInProgress); } }
        ...
        internal bool ShouldRouteVanillaCatalogSpawn { get { return !IsHost && lobby.HasValue && (sessionActive || ClientMapTransitionInProgress); } }
        private bool ClientMapTransitionInProgress { get { return clientMapLoadPending || clientMapLoadIssued || clientMapInstanceLoaded || clientMapSceneTransitionPending; } }
        internal bool AllowClientWorldClear()
        {
            if (!lobby.HasValue || IsHost) return true;
            SetStatus("Only the host can clear or reload the shared map.");
            return false;
        }
        internal void NotifyUnsupportedClientContextAction(string actionName)
        {
            SetStatus("This action is not synchronised yet. Ask the host to perform it; unsupported local edits are blocked.");
            Logger.LogInfo("[Connect][Input] Blocked unsynchronised client context action: " + SafeName(actionName) + ".");
        }
        ...
        private static string LocalGameVersion
        {
            get
            {
                string version = Application.version;
                return string.IsNullOrEmpty(version) ? ExpectedGameVersion : version.Trim();
            }
        }
        ...
        private void SendToConnection(Connection connection, WireMessage type, WireChannel channel, ushort peerId, byte[] payload, bool reliable) { transport.SendToClient(connection, Wire.Pack(type, channel, nonce, peerId, ++sequence, hostTick, ScopeWorldPayload(channel, worldEpoch, payload)), reliable); }
        private void SendToHost(WireMessage type, WireChannel channel, byte[] payload, bool reliable) { transport.SendToHost(Wire.Pack(type, channel, nonce, clientPeerId, ++sequence, hostTick, ScopeWorldPayload(channel, clientWorldEpoch, payload)), reliable); }
        private void Broadcast(WireMessage type, WireChannel channel, byte[] payload, bool reliable)
        {
            foreach (Peer peer in peers.Values)
            {
                if (peer == null || peer.Connection == null) continue;
                if (channel == WireChannel.Snapshot && (peer.MapStatus != PeerMapStatus.Playing || !string.Equals(peer.MapIdentity, activeMapIdentity, StringComparison.Ordinal))) continue;
                SendToConnection(peer.Connection, type, channel, peer.PeerId, payload, reliable);
            }
        }
        ...
        private sealed class Peer { internal ulong SteamId; internal ushort PeerId; internal Connection Connection; internal string Name; internal PeerMapStatus MapStatus; internal string MapIdentity; internal int BaselineCursor; internal int BaselineCyclesRemaining; internal float NextBaselineAt; }
        private struct SpawnRateWindow { internal float StartTime; internal int Count; }
        private struct RigSnapshotState
        {
